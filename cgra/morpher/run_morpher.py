...
##############################################################################################################################################
  if runmode == 'runall' or runmode == 'dfg_gen_only':
    ...
    # In light mode, DFG pass produces _PartPred_AGI_REMOVED_DFG.xml; rename to expected name
    if morpher_light and os.path.exists(kernel+'_PartPred_AGI_REMOVED_DFG.xml'):
      os.system('cp %s_PartPred_AGI_REMOVED_DFG.xml %s_PartPredDFG.xml' % (kernel, kernel))
    ...
    ###
    # In light mode, create a dummy mem_alloc file and copy DFG XML to mapper dir
    if morpher_light:
      mem_alloc_file = kernel + '_mem_alloc.txt'
      if not os.path.exists(mem_alloc_file):
        print('Light mode: creating dummy mem_alloc file\n')
        with open(mem_alloc_file, 'w') as mf:
          mf.write('variable,start_addr\n')
      os.system('cp %s %s' % (mem_alloc_file, MAPPER_KERNEL))
