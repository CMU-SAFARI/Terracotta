package terracotta_types_pkg;
  ...
  // EWMA bus-utilization throttling parameters
  localparam int TC_EWMA_W         = 16;   // EWMA value and per-tech threshold width
  localparam int TC_EWMA_ALPHA_W   = 4;    // Shift amount (alpha = 1/2^shift), range 0–15
  localparam int TC_EWMA_WINDOW_W  = 16;   // Window counter width (max window = 65535 cycles)

  // Per-technique priority for multi-tech arbitration
  localparam int TC_TECH_PRIO_W    = TC_TECH_W;  // 2 bits, values 0–3; higher = higher priority
  ...
  // [1:0] update_op: 2'b00=ZERO, 2'b01=INCR(+1), 2'b10=CONST(assign), 2'b11=PROB SET
  ...
  localparam int TC_NUM_CORES     = 4;
  localparam int TC_META_DEPTH    = 16 * TC_NUM_CORES;   // 64 entries per bank per tech
  localparam int TC_BANK_W        = TC_BG_W + TC_BA_W;   // 5 bits → 32 flat banks
  localparam int TC_NUM_BANKS     = (1 << TC_BANK_W);    // 32
  localparam int TC_META_TAG_W    = TC_SA_W + TC_ROW_W + TC_COL_W; // 26 bits (full tag)
  localparam int TC_META_IDX_W    = $clog2(TC_META_DEPTH);          // 6 bits
  localparam int TC_META_STORED_TAG_W = TC_META_TAG_W - TC_META_IDX_W; // 20 bits
