...
module UpdateArray #(
    ...
    // Per-tech metadata inputs (from MetadataTableBank read results)
    ...
    output logic [BANK_W-1:0]            rd_bank_id_o         [NUM_TECH],
    ...
    output logic [ROW_W+COL_W+SA_W-1:0]  wr_tag_o             [NUM_TECH],
    output logic [BANK_W-1:0]            wr_bank_id_o         [NUM_TECH]
);
    ...
    generate
        for (i = 0; i < NUM_TECH; i++) begin : g_upd
            UpdateUnit #(
                ...
                .CFG_W(CFG_W), .CFG_DEPTH(CFG_DEPTH), .TECH_W(TECH_W), .TECH_ID(i), .BANK_W(BANK_W)
            ) u_upd (
                .clk_i(clk_i), .reset_i(reset_i),
                .in_valid_i(inst_in_valid[i]), .in_ready_o(inst_in_ready[i]),
                .pkt_i(pkt_i),
                .metadata_valid_i(metadata_valid_i[i]), .metadata_in_i(metadata_in_i[i]),
                .metadata_tag_valid_o(metadata_tag_valid_o[i]),
                .metadata_tag_o(metadata_tag_o[i]),
                .rd_bank_id_o(rd_bank_id_o[i]),
