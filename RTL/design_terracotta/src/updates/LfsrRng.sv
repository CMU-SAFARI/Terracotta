...
#(
  parameter logic [16-1:0] SEED = 16'hACE1  // non-zero default seed
)(
  input  logic                     clk_i,
  input  logic                     reset_i,

  output logic [16-1:0]  rng_o   // current LFSR value
);
  ...
  always_ff @(posedge clk_i ) begin
    if (reset_i) begin
      ...
      lfsr_r[16-1] <= lfsr_r[0];                         // bit 15 ← feedback
      lfsr_r[16-2] <= lfsr_r[16-1] ^ lfsr_r[0]; // bit 14 ← b15 ^ fb (tap 15)
      lfsr_r[16-3] <= lfsr_r[16-2];             // bit 13 ← b14
      lfsr_r[16-4] <= lfsr_r[16-3] ^ lfsr_r[0]; // bit 12 ← b13 ^ fb (tap 13)
