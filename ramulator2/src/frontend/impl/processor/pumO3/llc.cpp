...
PuMO3LLC::PuMO3LLC(int latency, int size_bytes, int linesize_bytes, int associativity, int num_mshrs):
m_latency(latency), m_size_bytes(size_bytes), m_linesize_bytes(linesize_bytes), m_associativity(associativity), m_num_mshrs(num_mshrs) {
  m_logger = Logging::create_logger("PuMO3LLC");
  ...
  DEBUG_LOG(DPUMO3LLC, m_logger, "Index mask: {0:x}", m_index_mask);
  DEBUG_LOG(DPUMO3LLC, m_logger, "Index offset: {}",  m_index_offset);
  DEBUG_LOG(DPUMO3LLC, m_logger, "Tag offset: {}",    m_tag_offset);
};
...
void PuMO3LLC::tick() {
  ...
  it = m_pum_list.begin();
  while (it != m_pum_list.end()) {
    if (m_clk >= it->first) {
      if (!m_memory_system->send(it->second)) {
        it++;
      } else {
        it = m_pum_list.erase(it);
      }
    } else {
      it++;
    }
...
bool PuMO3LLC::send(Request req) {
  ...
  // If request is not a read or write, dirty out the destination cache line and queue the request
  if (req.type_id != Request::Type::Read && req.type_id != Request::Type::Write) {
    if (auto line_it = check_set_hit(set, req.dest_addr); line_it != set.end()) {
      line_it->dirty = true;
    }
    m_pum_list.push_back(std::make_pair(m_clk + m_latency, req));
    return true;
  }

  if (auto line_it = check_set_hit(set, req.addr); line_it != set.end()) {
    // Hit in the set
    DEBUG_LOG(DPUMO3LLC, m_logger, 
    ...
    if (mshr_it != m_mshrs.end()) {
      DEBUG_LOG(DPUMO3LLC, m_logger,  "MSHR Hit.", m_clk);
    ...
    if (m_mshrs.size() == m_num_mshrs) {
      DEBUG_LOG(DPUMO3LLC, m_logger,  "No MSHR entry available.", m_clk);
    ...
    if (!line_available) {
      DEBUG_LOG(DPUMO3LLC, m_logger,  "No cache line available in the set.", m_clk);
      return false;
    }
...
void PuMO3LLC::receive(Request& req) {
  if (req.type_id != Request::Type::Read && req.type_id != Request::Type::Write) {
    return;
  }
  ...
  DEBUG_LOG(DPUMO3LLC, m_logger, "[Clk={}] Request {} received.", m_clk, req.addr);
...
PuMO3LLC::CacheSet_t& PuMO3LLC::get_set(Addr_t addr) {
...
PuMO3LLC::CacheSet_t::iterator PuMO3LLC::allocate_line(CacheSet_t& set, Addr_t addr) {
...
bool PuMO3LLC::need_eviction(const CacheSet_t& set, Addr_t addr) {
...
void PuMO3LLC::evict_line(CacheSet_t& set, CacheSet_t::iterator victim_it) {
  DEBUG_LOG(DPUMO3LLC, m_logger,  "Evicting {}.", victim_it->addr);
  s_llc_eviction++;
  ...
  if (victim_it->dirty) {
    ...
    DEBUG_LOG(DPUMO3LLC, m_logger,  "Writeback Request will be issued at Clk={}.", m_clk + m_latency);
  }
...
PuMO3LLC::CacheSet_t::iterator PuMO3LLC::check_set_hit(CacheSet_t& set, Addr_t addr) {
...
PuMO3LLC::MSHR_t::iterator PuMO3LLC::check_mshr_hit(Addr_t addr) {
  auto mshr_it =
    std::find_if(
      m_mshrs.begin(), m_mshrs.end(),
...
void PuMO3LLC::serialize(std::string serialization_filename) {
...
void PuMO3LLC::deserialize(std::string serialization_filename) {
  ...
  while (std::getline(serialization_file, file_line)) {
    ...
    if(m_cache_sets.find(index) == m_cache_sets.end()){
      m_cache_sets.insert({index, std::list<PuMO3LLC::Line>()});
    }
