#ifndef     RAMULATOR_FRONTEND_PROCESSOR_PuMO3_LLC_H
#define     RAMULATOR_FRONTEND_PROCESSOR_PuMO3_LLC_H
...
DECLARE_DEBUG_FLAG(DPUMO3LLC);
// ENABLE_DEBUG_FLAG(DPUMO3LLC);

class PuMO3LLC : public Clocked<PuMO3LLC> {
  friend class PuMO3;
  struct Line {
    Addr_t addr = -1;
    Addr_t tag = -1;
  ...
  private:
    ...
    std::list<std::pair<Clk_t, Request>> m_pum_list;
  ...
  public:
    PuMO3LLC(int latency, int size_bytes, int linesize_bytes, int associativity, int num_mshrs);
...
#endif   // RAMULATOR_FRONTEND_PROCESSOR_PuMO3_LLC_H