...
class PuMO3 final : public IFrontEnd, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IFrontEnd, PuMO3, "PuMO3", "Simple timing model OoO processor frontend.")

  private:
    ITranslation*  m_translation;

    int m_num_cores = -1;
    std::vector<PuMO3Core*> m_cores;
    PuMO3LLC* m_llc;
  ...
  public:
    void init() override {
      ...
      m_num_iterations = param<int>("num_iterations").desc("Number of iterations that the frontend should execute.").required();
      std::string operation = param<std::string>("operation").desc("Operation type").required();

      // Create address translation module
      m_translation = create_child_ifce<ITranslation>();

      // Create the LLC
      m_llc = new PuMO3LLC(llc_latency, llc_capacity_per_core * m_num_cores, llc_linesize_bytes, llc_associativity, llc_num_mshr_per_core * m_num_cores);
      // m_llc->deserialize(serialization_filename);
      // m_llc->serialize(serialization_filename);

      // Create the cores
      for (int id = 0; id < m_num_cores; id++) {
        PuMO3Core* core = new PuMO3Core(id, ipc, depth, m_num_iterations, trace_list[id], operation, m_translation, m_llc, lat_hist_sensitivity, lat_dump_path);
        core->m_callback = [this](Request& req){return this->receive(req);} ;
        m_cores.push_back(core);
      }

      m_logger = Logging::create_logger("PuMO3");

      // Register the stats
      register_stat(m_num_iterations).name("num_iterations");
      ...
      register_stat(m_llc->s_llc_pum_ops).name("llc_pum_ops");
    ...
    void receive(Request& req) {
      ...
      // Handle RowClone request without involving the core's callback
