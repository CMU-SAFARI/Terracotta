...
void SimpleO3Core::InstWindow::insert(bool ready, Addr_t addr, Clk_t clk) {
...
Clk_t SimpleO3Core::InstWindow::set_ready(Addr_t addr) {
...
SimpleO3Core::SimpleO3Core(int id, int ipc, int depth, size_t num_expected_insts, uint64_t num_max_cycles, std::string trace_path, ITranslation* translation, SimpleO3LLC* llc, int lat_hist_sens, std::string& dump_path):
m_id(id), m_window(ipc, depth), m_trace(trace_path), m_num_expected_insts(num_expected_insts), m_num_max_cycles(num_max_cycles), m_translation(translation), m_llc(llc), m_lat_hist_sens(lat_hist_sens) {
...
void SimpleO3Core::dump_latency_histogram() {
  if (m_dump_path == "") {
    return;
  }
