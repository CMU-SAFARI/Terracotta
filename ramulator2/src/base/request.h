...
struct Request { 
  ...
  // Additional address for other purposes, e.g., RowClone
  Addr_t   ext_addr = -1;
  AddrVec_t   ext_addr_vec = {};

  Addr_t real_addr = -1;
  AddrVec_t real_addr_vec = {};

  Addr_t source1_addr = -1; 
  AddrVec_t source1_addr_vec = {};

  Addr_t source2_addr = -1;
  AddrVec_t source2_addr_vec = {};

  Addr_t dest_addr = -1;
  AddrVec_t dest_addr_vec = {};

  // Basic request id convention
  // 0 = Read, 1 = Write. The device spec defines all others
  // Introduces a new type of request for RowClone
  struct Type {
    enum : int {
      Read = 0, 
      Write,
  ...
  bool is_done = false; // Set by actions when the request is done
  ...
  Request(Addr_t addr, Addr_t ext_addr, int type, int source_id, std::function<void(Request&)> callback);
  Request(AddrVec_t addr_vec, AddrVec_t ext_addr_vec, int type, int source_id, std::function<void(Request&)> callback);
  Request(AddrVec_t addr_vec, AddrVec_t ext_addr_vec, AddrVec_t real_addr_vec, int type, int source_id, std::function<void(Request&)> callback);

  // Support for PuM requests
  Request(Addr_t source1_addr, Addr_t source2_addr, Addr_t dest_addr, int type, int source_id, std::function<void(Request&)> callback);
};
...
struct ReqBuffer {
  ...
  bool can_enqueue(int n=1) const {
    return buffer.size() + n <= max_size;
  }
