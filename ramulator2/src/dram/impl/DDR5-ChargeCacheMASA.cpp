...
class DDR5ChargeCacheMASA : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, DDR5ChargeCacheMASA, "DDR5ChargeCacheMASA", "DDR5 ChargeCache+MASA Device Model")
  private:
    int m_RH_radius = -1;
  ...
  public:
    struct Node : public DRAMNodeBase<DDR5ChargeCacheMASA> {
      Node(DDR5ChargeCacheMASA* dram, Node* parent, int level, int id) : DRAMNodeBase<DDR5ChargeCacheMASA>(dram, parent, level, id) {};

      // ChargeCache: update timing function that uses m_cc_timing_cons
      void update_timing_chargecache(int command, const AddrVec_t& addr_vec, int clk) {
        /************************************************
         *         Update Sibling Node Timing
         ***********************************************/
        auto* dram = static_cast<DDR5ChargeCacheMASA*>(m_spec);
  ...
  public:
    ...
    // ChargeCache: overloaded issue_command that routes ChargeCache hits to reduced timings
    ...
    // ChargeCache+MASA: get open rows from subarrays
    std::vector<int> get_open_rows(AddrVec_t addr_vec) override {
      int channel_id = addr_vec[m_levels["channel"]];
      int rank_id    = addr_vec[m_levels["rank"]];
      int bg_id      = addr_vec[m_levels["bankgroup"]];
      int bank_id    = addr_vec[m_levels["bank"]];
      int sa_id      = addr_vec[m_levels["subarray"]];

      auto* bank_node = m_channels[channel_id]->m_child_nodes[rank_id]
                         ->m_child_nodes[bg_id]->m_child_nodes[bank_id];

      std::vector<int> open_rows;
      if (sa_id >= 0) {
        // Get open rows from a specific subarray
        auto* sa_node = bank_node->m_child_nodes[sa_id];
        for (const auto& [row_id, state] : sa_node->m_row_state) {
          if (state == m_states("Opened")) {
            open_rows.push_back(row_id);
          }
        }
      } else {
        // Get open rows from all subarrays in the bank
    ...
    // ChargeCache+MASA: get open rows with subarray info for HCRAC
    // Returns pairs of (subarray_id, row_id)
    std::vector<std::pair<int, int>> get_open_rows_with_sa(AddrVec_t addr_vec) {
      int channel_id = addr_vec[m_levels["channel"]];
      int rank_id    = addr_vec[m_levels["rank"]];
      int bg_id      = addr_vec[m_levels["bankgroup"]];
      int bank_id    = addr_vec[m_levels["bank"]];
      ...
      std::vector<std::pair<int, int>> open_rows;
      if (sa_id >= 0) {
        // Get open rows from a specific subarray
        auto* sa_node = bank_node->m_child_nodes[sa_id];
        for (const auto& [row_id, state] : sa_node->m_row_state) {
          if (state == m_states("Opened")) {
            open_rows.push_back({sa_id, row_id});
          }
        }
      } else {
        // Get open rows from all subarrays in the bank
        for (auto* sa_node : bank_node->m_child_nodes) {
          for (const auto& [row_id, state] : sa_node->m_row_state) {
            if (state == m_states("Opened")) {
              open_rows.push_back({sa_node->m_node_id, row_id});
            }
          }
        }
      }
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      // ChargeCache: Set reduced timings
      ...
      // ChargeCache: populate m_cc_timing_cons with reduced ACT timings
      ...
      std::vector<TimingConsInitializer> initializer = {
        // Rank level: reduced ACT -> PREA/REF timings
        {.level = "rank", .preceding = {"ACT"}, .following = {"ACT"}, .latency = V("nRRDS")},          
        {.level = "rank", .preceding = {"ACT"}, .following = {"ACT"}, .latency = V("nFAW"), .window = 4},          
        ...
        // Bank level: reduced ACT -> REF/PRE timings
        {.level = "bank", .preceding = {"ACT"}, .following = {"REFsb", "RFMsb", "DRFMsb"}, .latency = m_cc_tRC},
        {.level = "bank", .preceding = {"ACT"}, .following = {"PRE", "PREsb"}, .latency = m_cc_tRAS},

        // Subarray level: reduced ACT timings
        {.level = "subarray", .preceding = {"ACT"}, .following = {"ACT"}, .latency = m_cc_tRC},
        {.level = "subarray", .preceding = {"ACT"}, .following = {"PRE_SA"}, .latency = m_cc_tRAS},
        {.level = "subarray", .preceding = {"ACT"}, .following = {"SA_SEL"}, .latency = m_cc_tRCD},
      };
    ...
    void set_preqs() {
      ...
      // Rank Preqs
      m_preqs[m_levels["rank"]][m_commands["REFab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCacheMASA>;
      m_preqs[m_levels["rank"]][m_commands["RFMab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCacheMASA>;
      m_preqs[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCacheMASA>;

      // Same-Bank Preqs.
      m_preqs[m_levels["rank"]][m_commands["REFsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCacheMASA>;
      m_preqs[m_levels["rank"]][m_commands["RFMsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCacheMASA>;
      m_preqs[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCacheMASA>;

      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Preq::Bank::RequireBankClosed<DDR5ChargeCacheMASA>;  
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Power::Bank::ACT<DDR5ChargeCacheMASA>;
      m_powers[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Power::Bank::PRE<DDR5ChargeCacheMASA>;
      m_powers[m_levels["bank"]][m_commands["RD"]]  = Lambdas::Power::Bank::RD<DDR5ChargeCacheMASA>;
      m_powers[m_levels["bank"]][m_commands["WR"]]  = Lambdas::Power::Bank::WR<DDR5ChargeCacheMASA>;

      m_powers[m_levels["rank"]][m_commands["RFMsb"]] = Lambdas::Power::Rank::RFMsb<DDR5ChargeCacheMASA>;
      m_powers[m_levels["rank"]][m_commands["RFMsb_end"]] = Lambdas::Power::Rank::RFMsb_end<DDR5ChargeCacheMASA>;

      m_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<DDR5ChargeCacheMASA>;
      m_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<DDR5ChargeCacheMASA>;
      m_powers[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Power::Rank::PREA<DDR5ChargeCacheMASA>;
      m_powers[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Power::Rank::REFab<DDR5ChargeCacheMASA>;
      m_powers[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5ChargeCacheMASA>;

      m_powers[m_levels["rank"]][m_commands["PREsb"]] = Lambdas::Power::Rank::PREsb<DDR5ChargeCacheMASA>;
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      Lambdas::Power::Rank::finalize_rank<DDR5ChargeCacheMASA>(rank_node, 0, AddrVec_t(), m_clk);
