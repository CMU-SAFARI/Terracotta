...
class TerracottaDDR5PRADA : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, TerracottaDDR5PRADA, "TerracottaDDR5PRADA", "Terracotta DDR5 PRADA Device Model")
  private:
    int m_RH_radius = -1;
   ...
   ***********************************************/
    ...
    inline static const ImplLUT m_command_scopes = LUT (
      m_commands, m_levels, {
        ...
        {"ACTocwl", "row"}, {"ACTwls", "row"}, {"ACTwl", "row"}, {"NOT", "row"},
      }
    );
    ...
    inline static const ImplLUT m_command_meta = LUT<DRAMCommandMeta> (
      m_commands, {
        ...
        {"ACTocwl",     {true,   false,   false,   false}},
        {"ACTwls",      {true,   false,   false,   false}},
        {"ACTwl",       {true,   false,   false,   false}},
  ...
  public:
    struct Node : public DRAMNodeBase<TerracottaDDR5PRADA> {
      Node(TerracottaDDR5PRADA* dram, Node* parent, int level, int id) : DRAMNodeBase<TerracottaDDR5PRADA>(dram, parent, level) {
        m_node_id = id;

        int num_cmds = m_commands.size();
        m_cmd_ready_clk.resize(num_cmds, -1);
        m_cmd_history.resize(num_cmds);
        for (int cmd = 0; cmd < num_cmds; cmd++) {
          int window = 0;
          for (const auto& t : dram->m_timing_cons[level][cmd]) {
            window = std::max(window, t.window);
          }
          for (const auto& t : dram->m_prada_timing_cons[level][cmd]) {
            window = std::max(window, t.window);
          }
          if (window != 0) {
        ...
        m_state = dram->m_init_states[m_level];

        // Recursively construct next levels
        int next_level = level + 1;
        int last_level = m_levels["row"];
        if (next_level == last_level) {
          return;
        } else {
          int next_level_size = dram->m_organization.count[next_level];
          if (next_level_size == 0) {
            return;
          } else {
            for (int i = 0; i < next_level_size; i++) {
              NodeType* child = new NodeType(dram, static_cast<NodeType*>(this), next_level, i);
      ...
      // timing update for PRADA requests
      void update_timing_prada(int command, const AddrVec_t& addr_vec, Clk_t clk) {
        /************************************************
         *         Update Sibling Node Timing
         ***********************************************/
        // m_prada_timing_cons contains an entry at m_level, command, choose that else choose it from m_spec->m_timing_cons
        auto* dram = static_cast<TerracottaDDR5PRADA*>(m_spec);

        if (m_node_id != addr_vec[m_level] && addr_vec[m_level] != -1) {
          for (const auto& t : dram->m_prada_timing_cons[m_level][command]) {
            if (!t.sibling) {
        ...
        for (auto child : m_child_nodes) {
          child->update_timing_prada(command, addr_vec, clk);
        }
      };

      // state update
      void update_states_prada(ReqBuffer::iterator req_it, Clk_t clk) {
        ...
        if (child_id == -1) {
          for (auto child : m_child_nodes) {
            child->update_states_prada(req_it, clk);
          }
        } else {
          m_child_nodes[child_id]->update_states_prada(req_it, clk);
        }
      };

      void update_powers_prada(ReqBuffer::iterator req_it, Clk_t clk) {
        ...
        if (m_spec->m_prada_powers[m_level][request_type]) {
          // update the power model at this level
          m_spec->m_prada_powers[m_level][request_type](static_cast<NodeType*>(this), req_it, clk);
        }
        if (!m_child_nodes.size()) {
          // stop recursion: updated all levels
          return; 
        ...
        if (child_id == -1) {
          for (auto child : m_child_nodes) {
            child->update_powers_prada(req_it, clk);
          }
        } else {
          m_child_nodes[child_id]->update_powers_prada(req_it, clk);
        }
      };

      int get_prada_preq_command(ReqBuffer::iterator req_it, Clk_t clk)
      {
        int child_id = req_it->addr_vec[m_level + 1];
        ...
        return m_child_nodes[child_id]->get_prada_preq_command(req_it, clk);
      }
    };
    ...
    using PradaPowerFunc_t = std::function<void (Node* node, ReqBuffer::iterator req_it, Clk_t clk)>;
    FuncMatrix<PradaPowerFunc_t>   m_prada_powers;
  ...
  public:
    ...
    int nACToc, nACTwls, nNOT, new_cmd_latency;

    TimingCons m_rowclone_timing_cons;
    TimingCons m_not_timing_cons;
    TimingCons m_prada_timing_cons;
  ...
  public:
    ...
    void issue_command(ReqBuffer::iterator& req_it) override {
      ...
      if (req_it->type_id == Request::Type::RowClone || 
        ...
        m_channels[channel_id]->update_timing_prada(command, addr_vec, m_clk);
        m_channels[channel_id]->update_powers(command, addr_vec, m_clk);
        m_channels[channel_id]->update_states_prada(req_it, m_clk);
        if (req_it->type_id == Request::Type::RowClone && command == req_it->final_command)
          ...
          req_it->is_done = true;
        else if (req_it->type_id == Request::Type::AND_OR && req_it->step_id == 3)
          req_it->is_done = true;
        else if (req_it->type_id == Request::Type::XOR_XNOR && req_it->step_id == 7)
          req_it->is_done = true;
        else if (req_it->type_id == Request::Type::NAND_NOR && req_it->step_id == 3)
          req_it->is_done = true;

        if (req_it->is_done)
          m_channels[channel_id]->update_powers_prada(req_it, m_clk);
      } else {
        m_channels[channel_id]->update_timing(command, addr_vec, m_clk);
        m_channels[channel_id]->update_powers(command, addr_vec, m_clk);
        m_channels[channel_id]->update_states(command, addr_vec, m_clk);
    ...
    void set_preq_command(ReqBuffer::iterator req_it) override {
      ...
      if (req_it->type_id != Request::Type::Read && req_it->type_id != Request::Type::Write)
        req_it->command = m_channels[channel_id]->get_prada_preq_command(req_it, m_clk);
      else
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      // PRADA parameters
      nACToc   = JEDEC_rounding_DDR5(5, tCK_ps);  // 5ns
      nACTwls  = m_timing_vals("nRAS") - JEDEC_rounding_DDR5(5, tCK_ps);  // 8ns
      nNOT     = JEDEC_rounding_DDR5(35, tCK_ps); // 35ns
      // new_cmd_latency: extra device-side latency (cycles) charged on PRADA's new
      // commands (ACTocwl/ACTwl/ACTwls/NOT). Models command-processing overhead at
      // the DRAM device; distinct from the memory-controller trigger latency
      // (Controller: controller_latency). The Terracotta variant uses a higher value
      // and also charges it on the per-bank same-command constraints, reflecting the
      // programmable command path's added overhead vs. the fixed custom controller.
      new_cmd_latency = 3;
      ...
      m_prada_timing_cons.resize(m_levels.size(), std::vector<std::vector<TimingConsEntry>>(m_commands.size()));
      ...
      std::vector<TimingConsInitializer> initializer = {
        ...
        {.level = "channel", .preceding = {"ACTocwl", "ACTwl", "ACTwls", "NOT"}, .following = all_commands, .latency = new_cmd_latency},

        /*** Rank (or different BankGroup) ***/
        /// RAS <-> RAS
        {.level = "rank", .preceding = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .following = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .latency = V("nRRDS")},
        // {.level = "rank", .preceding = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .following = {"PREA"}, .latency = V("nRAS")}, // Dropping this because it can never happen
        {.level = "rank", .preceding = {"NOT"}, .following = {"PREA"}, .latency = nNOT},

        /// RAS <-> REF 
        {.level = "rank", .preceding = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .following = {"REFab", "RFMab", "DRFMab"}, .latency = V("nRC")},
        {.level = "rank", .preceding = {"NOT"}, .following = {"REFab", "RFMab", "DRFMab"}, .latency = nNOT},
        {.level = "rank", .preceding = {"PRE"}, .following = {"REFab", "RFMab", "DRFMab"}, .latency = V("nRP")}, 

        /*** Same Bank Group ***/ 
        /// RAS <-> RAS
        {.level = "bankgroup", .preceding = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .following = {"ACT", "ACTocwl", "ACTwl", "ACTwls"}, .latency = V("nRRDL")},

        /*** Bank ***/ 
        {.level = "bank", .preceding = {"ACTwl"}, .following = {"NOT", "PRE", "PREsb"}, .latency = V("nWR") + new_cmd_latency},

        {.level = "bank", .preceding = {"ACTwls"}, .following = {"ACTwl"}, .latency = V("nRAS") / 2 + new_cmd_latency},
        {.level = "bank", .preceding = {"ACTwls"}, .following = {"NOT", "PRE", "PREsb"}, .latency = nACTwls + new_cmd_latency},

        {.level = "bank", .preceding = {"ACTocwl"}, .following = {"ACTwl"}, .latency = nACToc + new_cmd_latency},

        {.level = "bank", .preceding = {"ACT"}, .following = {"REFsb", "RFMsb", "DRFMsb"}, .latency = V("nRC")},
        {.level = "bank", .preceding = {"ACT"}, .following = {"ACTwl"}, .latency = nACToc + V("nRAS") / 2},
        {.level = "bank", .preceding = {"ACT"}, .following = {"NOT"}, .latency = V("nRAS")},
        {.level = "bank", .preceding = {"ACT"}, .following = {"PRE", "PREsb"}, .latency = V("nRAS")},
        
        {.level = "bank", .preceding = {"NOT"}, .following = {"ACT", "ACTocwl", "ACTwl", "ACTwls", "NOT", "PRE", "PREsb", "REFsb", "RFMsb", "DRFMsb"}, .latency = nNOT + new_cmd_latency},
        {.level = "bank", .preceding = {"PRE"}, .following = {"ACT", "ACTocwl", "ACTwl", "NOT", "ACTwls", "REFsb", "RFMsb", "DRFMsb"}, .latency = V("nRP")},
      };

      // Populate m_prada_timing_cons
      for (const auto& ts : initializer) {
        ...
        for (auto p_cmd_str : ts.preceding) {
          ...
          for (auto f_cmd_str : ts.following) {
            ...
            m_prada_timing_cons[level][p_cmd].push_back({f_cmd, ts.latency, ts.window, ts.is_sibling});
          }
        }
      }
    ...
    static void ACT(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
      ...
      if (cur_state == m_states["Closed"]) {
        ...
        spdlog::error("[Action::Bank::ACT] Invalid bank state for ACT command during PRADA operations!");
        std::exit(-1);
      }
    };

    static void ACTwl(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
      int cur_state = node->m_state;
      if (cur_state == m_states["Opened"]) {
        node->m_state = m_states["Two-Opened"];
      } else if (cur_state == m_states["Two-Opened"]) {
        node->m_state = m_states["Three-Opened"];
      } else {
        spdlog::error("[Action::Bank::ACT] Invalid bank state for ACT command during PRADA operations!");
        std::exit(-1);
      }
    ...
    static void ACTwls(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
      int cur_state = node->m_state;
      if (cur_state == m_states["Opened"]) {
        node->m_state = m_states["Two-Opened"];
      } else if (cur_state == m_states["Two-Opened"]) {
    ...
    static void ACTocwl(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
      int cur_state = node->m_state;
      if (cur_state == m_states["Closed"]) {
        node->m_row_state[req_it->source1_addr_vec[m_levels["row"]]] = m_states["Opened"];
        node->m_state = m_states["Opened"];
    ...
    static void NOT(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
      int cur_state = node->m_state;
      if (cur_state == m_states["Opened"] || cur_state == m_states["Three-Opened"]) {
        node->m_state = m_states["Not"];
      } else {
        spdlog::error("[Action::Bank::NOT] Invalid bank state for NOT command during PRADA operations!");
        std::exit(-1);
      }
    };

    static void PRE(Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
    ...
    void set_actions() {
      ...
      // m_actions
      // Rank Actions
      m_actions[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Action::Rank::PREab<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Action::Rank::REFab<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Action::Rank::REFab_end<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Action::Rank::REFab<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Action::Rank::REFab_end<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Action::Rank::REFab<TerracottaDDR5PRADA>;
      m_actions[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Action::Rank::REFab_end<TerracottaDDR5PRADA>;
      
      // Same-Bank Actions.
      m_actions[m_levels["bankgroup"]][m_commands["PREsb"]] = Lambdas::Action::BankGroup::PREsb<TerracottaDDR5PRADA>;

      // We call update_timing for the banks in other BGs here
      m_actions[m_levels["bankgroup"]][m_commands["REFsb"]]  = Lambdas::Action::BankGroup::REFsb<TerracottaDDR5PRADA>;
      m_actions[m_levels["bankgroup"]][m_commands["REFsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<TerracottaDDR5PRADA>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb"]]  = Lambdas::Action::BankGroup::REFsb<TerracottaDDR5PRADA>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<TerracottaDDR5PRADA>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb"]] = Lambdas::Action::BankGroup::REFsb<TerracottaDDR5PRADA>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb_end"]] = Lambdas::Action::BankGroup::REFsb_end<TerracottaDDR5PRADA>;

      // Bank actions
      m_actions[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Action::Bank::ACT<TerracottaDDR5PRADA>;
      m_actions[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Action::Bank::PRE<TerracottaDDR5PRADA>;
      m_actions[m_levels["bank"]][m_commands["RDA"]] = Lambdas::Action::Bank::PRE<TerracottaDDR5PRADA>;
      m_actions[m_levels["bank"]][m_commands["WRA"]] = Lambdas::Action::Bank::PRE<TerracottaDDR5PRADA>;

      // m_rowclone_actions
      m_rowclone_actions[m_levels["bank"]][m_commands["ACT"]] = ACT;
      m_rowclone_actions[m_levels["bank"]][m_commands["ACTwl"]] = ACTwl;
      m_rowclone_actions[m_levels["bank"]][m_commands["PRE"]] = PRE;

      // m_not_actions
      m_not_actions[m_levels["bank"]][m_commands["ACT"]] = ACT;
      m_not_actions[m_levels["bank"]][m_commands["ACTwl"]] = ACTwl;
      m_not_actions[m_levels["bank"]][m_commands["NOT"]] = NOT;
      m_not_actions[m_levels["bank"]][m_commands["PRE"]] = PRE;

      // m_andor_actions
      m_andor_actions[m_levels["bank"]][m_commands["ACT"]] = ACT;
      m_andor_actions[m_levels["bank"]][m_commands["ACTocwl"]] = ACTocwl;
      m_andor_actions[m_levels["bank"]][m_commands["ACTwl"]] = ACTwl;
      m_andor_actions[m_levels["bank"]][m_commands["ACTwls"]] = ACTwls;
      m_andor_actions[m_levels["bank"]][m_commands["PRE"]] = PRE;

      // m_nand_nor_actions
      m_nandnor_actions[m_levels["bank"]][m_commands["ACT"]] = ACT;
      m_nandnor_actions[m_levels["bank"]][m_commands["ACTocwl"]] = ACTocwl;
      m_nandnor_actions[m_levels["bank"]][m_commands["ACTwl"]] = ACTwl;
      m_nandnor_actions[m_levels["bank"]][m_commands["ACTwls"]] = ACTwls;
      m_nandnor_actions[m_levels["bank"]][m_commands["NOT"]] = NOT;
      m_nandnor_actions[m_levels["bank"]][m_commands["PRE"]] = PRE;

      // m_xor_xnor_actions 
      m_xorxnor_actions[m_levels["bank"]][m_commands["ACT"]] = ACT;
      m_xorxnor_actions[m_levels["bank"]][m_commands["ACTocwl"]] = ACTocwl;
      m_xorxnor_actions[m_levels["bank"]][m_commands["ACTwl"]] = ACTwl;
      m_xorxnor_actions[m_levels["bank"]][m_commands["ACTwls"]] = ACTwls;
      m_xorxnor_actions[m_levels["bank"]][m_commands["NOT"]] = NOT;
      m_xorxnor_actions[m_levels["bank"]][m_commands["PRE"]] = PRE;
    };
    ...
    void set_preqs() {
      ...
      // m_preqs
      // Rank Preqs
      m_preqs[m_levels["rank"]][m_commands["REFab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<TerracottaDDR5PRADA>;
      m_preqs[m_levels["rank"]][m_commands["RFMab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<TerracottaDDR5PRADA>;
      m_preqs[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<TerracottaDDR5PRADA>;

      // Same-Bank Preqs.
      m_preqs[m_levels["rank"]][m_commands["REFsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<TerracottaDDR5PRADA>;
      m_preqs[m_levels["rank"]][m_commands["RFMsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<TerracottaDDR5PRADA>;
      m_preqs[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Preq::Rank::RequireSameBanksClosed<TerracottaDDR5PRADA>;

      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["RD"]] = Lambdas::Preq::Bank::RequireRowOpen<TerracottaDDR5PRADA>;
      m_preqs[m_levels["bank"]][m_commands["WR"]] = Lambdas::Preq::Bank::RequireRowOpen<TerracottaDDR5PRADA>;
      m_preqs[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Preq::Bank::RequireRowOpen<TerracottaDDR5PRADA>;
      m_preqs[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Preq::Bank::RequireBankClosed<TerracottaDDR5PRADA>;

      // m_rowclone_preqs
      m_rowclone_preqs[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        ...
        switch(node->m_state) {
          ...
          case m_states["Opened"]: {
            if (node->m_row_state.find(source_addr[m_levels["row"]]) != node->m_row_state.end())
              return m_commands["ACTwl"];
            else
              return m_commands["PRE"];
          }
          case m_states["Two-Opened"]: return m_commands["PRE"];
      ...
      // m_not_preqs
      m_not_preqs[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        int command = req_it->final_command;
        ...
        if (step_id == 0) {
          ...
          switch(node->m_state) {
            ...
            case m_states["Two-Opened"]: return m_commands["PRE"];
            case m_states["Refreshing"]: return m_commands["ACT"];
            case m_states["Not"]: return m_commands["PRE"];
            default:
              throw std::runtime_error("[Preq::Bank::ACT] Invalid bank state during Copy in NOT!");
              std::exit(-1);
          }
        } else if (step_id == 1) {
          // Perform NOT operation on the destination row
          switch(node->m_state) {
            case m_states["Closed"]: return m_commands["ACT"];
            case m_states["Opened"]: {
              if (node->m_row_state.find(source_addr[m_levels["row"]]) != node->m_row_state.end())
                return m_commands["NOT"];
              else
                return m_commands["PRE"];
            }
            case m_states["Not"]: return m_commands["PRE"];
            case m_states["Refreshing"]: return m_commands["ACT"];
            default:
              throw std::runtime_error("[Preq::Bank::ACT] Invalid bank state during NOT!");
              std::exit(-1);
          }
        }
      };
      ...
      // m_andor_preqs
      m_andor_preqs[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        int command = req_it->final_command;

        AddrVec_t source1_addr = req_it->source1_addr_vec;
        AddrVec_t source2_addr = req_it->source2_addr_vec;
        AddrVec_t dest_addr = req_it->dest_addr_vec;
        int step_id  = req_it->step_id;

        if (step_id == 0 || step_id == 1) {
          switch(node->m_state) {
            case m_states["Closed"]: return m_commands["ACT"];
            case m_states["Opened"]: {
              if (node->m_row_state.find(source1_addr[m_levels["row"]]) != node->m_row_state.end())
                return m_commands["ACTwl"];
              else
                return m_commands["PRE"];
            }
            case m_states["Two-Opened"]: return m_commands["PRE"];
            case m_states["Three-Opened"]: return m_commands["PRE"];
            case m_states["Refreshing"]: return m_commands["ACT"];
            default:
              throw std::runtime_error("[Preq::Bank::ACT] Invalid bank state during RowClone!");
              std::exit(-1);
          ...
          switch(node->m_state) {
            case m_states["Closed"]: return m_commands["ACTocwl"];
            case m_states["Opened"]: {
              if (node->m_row_state.find(source1_addr[m_levels["row"]]) != node->m_row_state.end())
                return m_commands["ACTwl"];
              else
            ...
            case m_states["Two-Opened"]: {
              if (node->m_row_state.find(source1_addr[m_levels["row"]]) != node->m_row_state.end())
                return m_commands["ACTwls"];
              else
                return m_commands["PRE"];
            }
            case m_states["Three-Opened"]: return m_commands["PRE"];
      ...
      // m_nandnor_preqs
      m_nandnor_preqs[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        int command = req_it->final_command;
        ...
        if (step_id == 0 || step_id == 1) {
          ...
          switch(node->m_state) {
            ...
            case m_states["Three-Opened"]: return m_commands["NOT"];
            case m_states["Not"]: return m_commands["PRE"];
            case m_states["Refreshing"]: return m_commands["ACT"];
            default:
              throw std::runtime_error("[Preq::Bank::ACT] Invalid bank state during RowClone!");
      ...
      m_xorxnor_preqs[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        ...
        } else if (step_id == 1 || step_id == 2 || step_id == 3) {
          // Perform multi-copy
          switch(node->m_state) {
            case m_states["Closed"]: return m_commands["ACT"];
            case m_states["Opened"]: {
            ...
            case m_states["Two-Opened"]: return m_commands["ACTwl"];
            case m_states["Three-Opened"]: return m_commands["PRE"];
            case m_states["Refreshing"]: return m_commands["ACT"];
            default:
              throw std::runtime_error("[Preq::Bank::ACT] Invalid bank state during RowClone!");
    ...
    void set_rowhits() {
      ...
      m_rowhits[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowHit::Bank::RDWR<TerracottaDDR5PRADA>;
      m_rowhits[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowHit::Bank::RDWR<TerracottaDDR5PRADA>;
    }
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowOpen::Bank::RDWR<TerracottaDDR5PRADA>;
      m_rowopens[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowOpen::Bank::RDWR<TerracottaDDR5PRADA>;
    }
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Power::Bank::ACT<TerracottaDDR5PRADA>;
      m_powers[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Power::Bank::PRE<TerracottaDDR5PRADA>;
      m_powers[m_levels["bank"]][m_commands["RD"]]  = Lambdas::Power::Bank::RD<TerracottaDDR5PRADA>;
      m_powers[m_levels["bank"]][m_commands["WR"]]  = Lambdas::Power::Bank::WR<TerracottaDDR5PRADA>;

      // m_powers[m_levels["rank"]][m_commands["REFsb"]] = Lambdas::Power::Rank::REFsb<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["REFsb_end"]] = Lambdas::Power::Rank::REFsb_end<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["RFMsb"]] = Lambdas::Power::Rank::RFMsb<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["RFMsb_end"]] = Lambdas::Power::Rank::RFMsb_end<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Power::Rank::REFsb<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb_end"]] = Lambdas::Power::Rank::REFsb_end<TerracottaDDR5PRADA>;

      m_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["ACTocwl"]] = Lambdas::Power::Rank::ACT<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["ACTwl"]] = Lambdas::Power::Rank::ACT<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["ACTwls"]] = Lambdas::Power::Rank::ACT<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Power::Rank::PREA<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Power::Rank::REFab<TerracottaDDR5PRADA>;
      m_powers[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Power::Rank::REFab_end<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Power::Rank::REFab<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Power::Rank::REFab_end<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Power::Rank::REFab<TerracottaDDR5PRADA>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Power::Rank::REFab_end<TerracottaDDR5PRADA>;

      m_powers[m_levels["rank"]][m_commands["PREsb"]] = Lambdas::Power::Rank::PREsb<TerracottaDDR5PRADA>;
      
      m_prada_powers.resize(m_levels.size(), std::vector<PradaPowerFunc_t>(m_requests.size()));

      m_prada_powers[m_levels["bank"]][m_requests["rowclone"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        Lambdas::Power::Bank::debug<TerracottaDDR5PRADA>(node, "Incrementing RowClone count", clk);
        node->m_spec->m_power_stats[Lambdas::Power::Bank::get_flat_rank_id<TerracottaDDR5PRADA>(node)].req_counters[TerracottaDDR5PRADA::m_reqs_counted("rowclone")] += 1;
      };
      m_prada_powers[m_levels["bank"]][m_requests["not"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        Lambdas::Power::Bank::debug<TerracottaDDR5PRADA>(node, "Incrementing not count", clk);
        node->m_spec->m_power_stats[Lambdas::Power::Bank::get_flat_rank_id<TerracottaDDR5PRADA>(node)].req_counters[TerracottaDDR5PRADA::m_reqs_counted("not")] += 1;
      };
      m_prada_powers[m_levels["bank"]][m_requests["andor"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        Lambdas::Power::Bank::debug<TerracottaDDR5PRADA>(node, "Incrementing andor count", clk);
        node->m_spec->m_power_stats[Lambdas::Power::Bank::get_flat_rank_id<TerracottaDDR5PRADA>(node)].req_counters[TerracottaDDR5PRADA::m_reqs_counted("andor")] += 1;
      };
      m_prada_powers[m_levels["bank"]][m_requests["nandnor"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        Lambdas::Power::Bank::debug<TerracottaDDR5PRADA>(node, "Incrementing nandnor count", clk);
        node->m_spec->m_power_stats[Lambdas::Power::Bank::get_flat_rank_id<TerracottaDDR5PRADA>(node)].req_counters[TerracottaDDR5PRADA::m_reqs_counted("nandnor")] += 1;
      };
      m_prada_powers[m_levels["bank"]][m_requests["xorxnor"]] = [](Node* node, ReqBuffer::iterator req_it, Clk_t clk) {
        Lambdas::Power::Bank::debug<TerracottaDDR5PRADA>(node, "Incrementing xorxnor count", clk);
        node->m_spec->m_power_stats[Lambdas::Power::Bank::get_flat_rank_id<TerracottaDDR5PRADA>(node)].req_counters[TerracottaDDR5PRADA::m_reqs_counted("xorxnor")] += 1;
      };
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      Lambdas::Power::Rank::finalize_rank<TerracottaDDR5PRADA>(rank_node, 0, AddrVec_t(), m_clk);
      ...
      double ap_seq_energy = (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) * TS("nRAS")
                                      + (VE("VDD") * (CE("IDD0") - CE("IDD2N")) + VE("VPP") * (CE("IPP0") - CE("IPP2N"))) * TS("nRP");

      double ap_power = ap_seq_energy / (TS("nRAS") + TS("nRP"));

      // ACT-ACTwl-PRE
      double copy_power = ap_power * 1.15;
      double copy_time = (nACToc + TS("nRAS") / 2) + TS("nWR") + TS("nRP");
      // ACT-ACTwl-NOT-PRE
      double not_power = ap_power * 1.08;
      double not_time = TS("nRAS") + nNOT + TS("nRP");
      // ACTocwl-ACTwl-ACTwls-PRE
      double tra_power = ap_power * 1.36;
      double tra_time = nACToc + new_cmd_latency + nACTwls + TS("nRP");
      // ACTocwl-ACTwl-ACTwls-NOT-PRE
      double tra_not_power = ap_power * 1.294;
      double tra_not_time = nACToc + new_cmd_latency + nACTwls + nNOT + TS("nRP");
      // ACT-ACTwl-ACTwl-PRE
      double two_copy_power = ap_power * 1.044;
      double two_copy_time = (nACToc + TS("nRAS") / 2) + TS("nWR") + TS("nWR") + TS("nRP");

      double rowclone_req_energy = rank_stats.req_counters[m_reqs_counted("rowclone")] 
                                      * (copy_power * copy_time) * tCK_ns / 1E3;
      double not_req_energy = rank_stats.req_counters[m_reqs_counted("not")] 
                                      * (copy_power * copy_time 
                                        + not_power * not_time) * tCK_ns / 1E3;
      double andor_req_energy = rank_stats.req_counters[m_reqs_counted("andor")]
                                      * (copy_power * copy_time
                                        + copy_power * copy_time
                                        + tra_power * tra_time) * tCK_ns / 1E3;
      double nandnor_req_energy = rank_stats.req_counters[m_reqs_counted("nandnor")]
                                      * (copy_power * copy_time
                                        + copy_power * copy_time
                                        + tra_not_power * tra_not_time) * tCK_ns / 1E3;
      double xorxnor_req_energy = rank_stats.req_counters[m_reqs_counted("xorxnor")]
                                      * (copy_power * copy_time
                                        + two_copy_power * two_copy_time
                                        + two_copy_power * two_copy_time
                                        + two_copy_power * two_copy_time
                                        + tra_power * tra_time
                                        + tra_not_power * tra_not_time
                                        + tra_power * tra_time) * tCK_ns / 1E3;
