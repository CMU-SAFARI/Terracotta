...
class DDR5MoPAC : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, DDR5MoPAC, "DDR5MoPAC", "DDR5 with Modified Page Access Control")
  private:
    int m_RH_radius = -1;
...
/************************************************
  ...
  public:
    struct Node : public DRAMNodeBase<DDR5MoPAC> {
      Node(DDR5MoPAC* dram, Node* parent, int level, int id) : DRAMNodeBase<DDR5MoPAC>(dram, parent, level, id) {};

      // update timing function that uses m_mopac_timing_vals
      void update_timing_mopac(int command, const AddrVec_t& addr_vec, int clk) {
        /************************************************
         *         Update Sibling Node Timing
         ***********************************************/
        // m_mopac_timing_cons contains an entry at m_level, command, choose that else choose it from m_spec->m_timing_cons
        auto* dram = static_cast<DDR5MoPAC*>(m_spec);
        const auto& timing_cons = dram->m_mopac_timing_cons[m_level][command].size() ? dram->m_mopac_timing_cons[m_level][command] : dram->m_timing_cons[m_level][command];
        ...
        for (auto child : m_child_nodes) {
          child->update_timing_mopac(command, addr_vec, clk);
        }
      };

      void update_powers_mopac(int command, const AddrVec_t& addr_vec, Clk_t clk) {
        auto* dram = static_cast<DDR5MoPAC*>(m_spec);

        if (!dram->m_drampower_enable)
          return;

        if (dram->m_mopac_powers[m_level][command]) {
          dram->m_mopac_powers[m_level][command](this, command, addr_vec, clk);
        }
        if (m_level == dram->m_command_scopes[command] || !m_child_nodes.size()) {
          // stop recursion: updated all levels
          return; 
        }
        ...
        if (child_id == -1) {
          for (auto child : m_child_nodes) {
            child->update_powers_mopac(command, addr_vec, clk);
          }
        } else {
          m_child_nodes[child_id]->update_powers_mopac(command, addr_vec, clk);
        }
      };
    };
    ...
    // MoPAC related
    FuncMatrix<PowerFunc_t<Node>>   m_mopac_powers;
    // MoPAC related
    ...
    std::vector<size_t> s_total_act_mopac_cycles;
    std::vector<size_t> s_total_pre_cycles;
    std::vector<size_t> s_total_pre_mopac_cycles;
  ...
  public:
    ...
    // MoPAC related
    TimingCons m_mopac_timing_cons;
    int m_mopac_tRAS = -1;
    int m_mopac_tRP  = -1;
    // MoPAC related
  ...
  public:
    ...
    void issue_command(ReqBuffer::iterator& req_it) override {
      ...
      // If req_it is marked, we use our regular timing parameters
      // else the special timing parameters
      if (req_it->scratchpad[0] == 1) {
        m_channels[channel_id]->update_timing_mopac(command, addr_vec, m_clk);
        m_channels[channel_id]->update_powers_mopac(command, addr_vec, m_clk);
      } else {
      ...
      // We do not do anything special here either
  ...
  private:
    void set_organization() {
      ...
      s_total_act_mopac_cycles.resize(num_channels * num_ranks, 0);
      s_total_pre_cycles.resize(num_channels * num_ranks, 0);
      s_total_pre_mopac_cycles.resize(num_channels * num_ranks, 0);
      ...
      for (int r = 0; r < num_channels * num_ranks; r++) {
        ...
        register_stat(s_total_act_mopac_cycles[r]).name("total_act_mopac_cycles_rank{}", r);
        register_stat(s_total_pre_cycles[r]).name("total_pre_cycles_rank{}", r);
        register_stat(s_total_pre_mopac_cycles[r]).name("total_pre_mopac_cycles_rank{}", r);
    ...
    void set_timing_vals() {
      ...
      if (tRAS > 0) {
        ...
        std::printf("[DDR5-MoPAC] tRAS (%d) provided:\n", tRAS);
        std::printf("\tnRAS: %d\n", m_timing_vals("nRAS"));
        std::printf("\tnRC: %d\n", m_timing_vals("nRC"));
      }
      ...
      // populate m_mopac_timing_cons
      m_mopac_tRAS = V("nRAS");
      m_mopac_tRP  = V("nRP");

      m_mopac_timing_cons.resize(m_levels.size(), std::vector<std::vector<TimingConsEntry>>(m_commands.size()));
      ...
      for (const auto& ts : initializer) {
        ...
        for (auto p_cmd_str : ts.preceding) {
          ...
          for (auto f_cmd_str : ts.following) {
            ...
            m_mopac_timing_cons[level][p_cmd].push_back({f_cmd, ts.latency, ts.window, ts.is_sibling});
          }
        }
      }
      #undef V
      ...
      if (prac_enabled) {
        std::cout << "[Ramulator::DDR5-MoPAC] <PRAC ENABLED> Timings are updated automatically. " << std::endl;
      ...
      std::cout << "[Ramulator::DDR5-MoPAC] nVRR: " << m_timing_vals("nVRR") << std::endl;
      std::cout << "[Ramulator::DDR5-MoPAC] nRFM1: " << m_timing_vals("nRFM1") << std::endl;
      std::cout << "[Ramulator::DDR5-MoPAC] nRFMsb: " << m_timing_vals("nRFMsb") << std::endl;
    ...
    void set_actions() {
      ...
      // Rank Actions
      m_actions[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Action::Rank::PREab<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Action::Rank::REFab<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Action::Rank::REFab<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Action::Rank::REFab<DDR5MoPAC>;
      m_actions[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5MoPAC>;

      // Same-Bank Actions.
      m_actions[m_levels["bankgroup"]][m_commands["PREsb"]] = Lambdas::Action::BankGroup::PREsb<DDR5MoPAC>;

      // We call update_timing for the banks in other BGs here
      m_actions[m_levels["bankgroup"]][m_commands["REFsb"]]  = Lambdas::Action::BankGroup::REFsb<DDR5MoPAC>;
      m_actions[m_levels["bankgroup"]][m_commands["REFsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<DDR5MoPAC>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb"]]  = Lambdas::Action::BankGroup::REFsb<DDR5MoPAC>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<DDR5MoPAC>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb"]] = Lambdas::Action::BankGroup::REFsb<DDR5MoPAC>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb_end"]] = Lambdas::Action::BankGroup::REFsb_end<DDR5MoPAC>;

      // Bank actions
      m_actions[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Action::Bank::ACT<DDR5MoPAC>;
      m_actions[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Action::Bank::PRE<DDR5MoPAC>;
      m_actions[m_levels["bank"]][m_commands["RDA"]] = Lambdas::Action::Bank::PRE<DDR5MoPAC>;
      m_actions[m_levels["bank"]][m_commands["WRA"]] = Lambdas::Action::Bank::PRE<DDR5MoPAC>;
      m_actions[m_levels["bank"]][m_commands["VRR"]] = Lambdas::Action::Bank::VRR<DDR5MoPAC>;
      m_actions[m_levels["bank"]][m_commands["VRR_end"]] = Lambdas::Action::Bank::VRR_end<DDR5MoPAC>;
    };
    ...
    void set_preqs() {
      ...
      // Rank Preqs
      m_preqs[m_levels["rank"]][m_commands["REFab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5MoPAC>;
      m_preqs[m_levels["rank"]][m_commands["RFMab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5MoPAC>;
      m_preqs[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5MoPAC>;
      m_preqs[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5MoPAC>;

      // Same-Bank Preqs.
      m_preqs[m_levels["rank"]][m_commands["REFsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5MoPAC>;
      m_preqs[m_levels["rank"]][m_commands["RFMsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5MoPAC>;
      m_preqs[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5MoPAC>;

      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["RD"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5MoPAC>;
      m_preqs[m_levels["bank"]][m_commands["WR"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5MoPAC>;
      m_preqs[m_levels["bank"]][m_commands["VRR"]] = Lambdas::Preq::Bank::RequireBankClosed<DDR5MoPAC>;
      m_preqs[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5MoPAC>;
      m_preqs[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Preq::Bank::RequireBankClosed<DDR5MoPAC>;
    ...
    void set_rowhits() {
      ...
      m_rowhits[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowHit::Bank::RDWR<DDR5MoPAC>;
      m_rowhits[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowHit::Bank::RDWR<DDR5MoPAC>;
    }
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowOpen::Bank::RDWR<DDR5MoPAC>;
      m_rowopens[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowOpen::Bank::RDWR<DDR5MoPAC>;
    }
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Power::Bank::ACT<DDR5MoPAC>;
      m_powers[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Power::Bank::PRE<DDR5MoPAC>;
      m_powers[m_levels["bank"]][m_commands["RD"]]  = Lambdas::Power::Bank::RD<DDR5MoPAC>;
      m_powers[m_levels["bank"]][m_commands["WR"]]  = Lambdas::Power::Bank::WR<DDR5MoPAC>;
      m_powers[m_levels["bank"]][m_commands["VRR"]]  = Lambdas::Power::Bank::VRR<DDR5MoPAC>;

      // m_powers[m_levels["rank"]][m_commands["REFsb"]] = Lambdas::Power::Rank::REFsb<DDR5MoPAC>;
      // m_powers[m_levels["rank"]][m_commands["REFsb_end"]] = Lambdas::Power::Rank::REFsb_end<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["RFMsb"]] = Lambdas::Power::Rank::RFMsb<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["RFMsb_end"]] = Lambdas::Power::Rank::RFMsb_end<DDR5MoPAC>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Power::Rank::REFsb<DDR5MoPAC>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb_end"]] = Lambdas::Power::Rank::REFsb_end<DDR5MoPAC>;

      m_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Power::Rank::PREA<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Power::Rank::REFab<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Power::Rank::RFMab<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Power::Rank::RFMab_end<DDR5MoPAC>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Power::Rank::REFab<DDR5MoPAC>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["VRR"]] = Lambdas::Power::Rank::VRR<DDR5MoPAC>;
      m_powers[m_levels["rank"]][m_commands["VRR_end"]] = Lambdas::Power::Rank::VRR_end<DDR5MoPAC>;

      m_powers[m_levels["rank"]][m_commands["PREsb"]] = Lambdas::Power::Rank::PREsb<DDR5MoPAC>;

      m_mopac_powers.resize(m_levels.size(), std::vector<PowerFunc_t<Node>>(m_commands.size()));

      m_mopac_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<DDR5MoPAC>;
      m_mopac_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<DDR5MoPAC>;

      m_mopac_powers[m_levels["bank"]][m_commands["ACT"]] = [](Node* node, int cmd, const AddrVec_t& addr, Clk_t clk) {
        Lambdas::Power::Bank::debug<DDR5MoPAC>(node, "Incrementing ACTmopac counter.", clk);
        int rank_id = Lambdas::Power::Bank::get_flat_rank_id<DDR5MoPAC>(node);
        node->m_spec->m_power_stats[rank_id].cmd_counters[node->m_spec->m_cmds_counted("ACTmopac")]++;
      };
      m_mopac_powers[m_levels["bank"]][m_commands["PRE"]] = [](Node* node, int cmd, const AddrVec_t& addr, Clk_t clk) {
        Lambdas::Power::Bank::debug<DDR5MoPAC>(node, "Incrementing PREmopac counter.", clk);
        int rank_id = Lambdas::Power::Bank::get_flat_rank_id<DDR5MoPAC>(node);
        node->m_spec->m_power_stats[rank_id].cmd_counters[node->m_spec->m_cmds_counted("PREmopac")]++;
      };
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      Lambdas::Power::Rank::finalize_rank<DDR5MoPAC>(rank_node, 0, AddrVec_t(), m_clk);
      ...
      double act_mopac_cmd_energy  = (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) 
                                      * rank_stats.cmd_counters[m_cmds_counted("ACTmopac")] * m_mopac_tRAS * tCK_ns / 1E3;
      ...
      double pre_mopac_cmd_energy  = (VE("VDD") * (CE("IDD0") - CE("IDD2N")) + VE("VPP") * (CE("IPP0") - CE("IPP2N"))) 
                                      * rank_stats.cmd_counters[m_cmds_counted("PREmopac")] * m_mopac_tRP * tCK_ns / 1E3;
      ...
      rank_stats.total_cmd_energy = act_cmd_energy 
                                    + act_mopac_cmd_energy
                                    + pre_cmd_energy 
                                    + pre_mopac_cmd_energy
                                    + rd_cmd_energy
                                    + wr_cmd_energy 
                                    + ref_cmd_energy
                                    + rfmsb_cmd_energy
      ...
      s_total_act_mopac_cycles[rank_stats.rank_id] = rank_stats.cmd_counters[m_cmds_counted("ACTmopac")] * m_mopac_tRAS;
      s_total_pre_cycles[rank_stats.rank_id] = rank_stats.cmd_counters[m_cmds_counted("PRE")] * TS("nRP");
      s_total_pre_mopac_cycles[rank_stats.rank_id] = rank_stats.cmd_counters[m_cmds_counted("PREmopac")] * m_mopac_tRP;
