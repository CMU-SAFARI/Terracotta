...
class DDR5ChargeCache : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, DDR5ChargeCache, "DDR5ChargeCache", "DDR5-ChargeCache Device Model")
  private:
    int m_RH_radius = -1;

    // ChargeCache parameters
    float m_cc_trcd_frac = 0.65;
    float m_cc_tras_frac = 0.75;
    // ChargeCache parameters
  ...
  public:
    struct Node : public DRAMNodeBase<DDR5ChargeCache> {
      Node(DDR5ChargeCache* dram, Node* parent, int level, int id) : DRAMNodeBase<DDR5ChargeCache>(dram, parent, level, id) {};

      // update timing function that uses m_cc_timing_vals
      void update_timing_chargecache(int command, const AddrVec_t& addr_vec, int clk) {
        /************************************************
         *         Update Sibling Node Timing
         ***********************************************/
        // m_cc_timing_cons contains an entry at m_level, command, choose that else choose it from m_spec->m_timing_cons
        auto* dram = static_cast<DDR5ChargeCache*>(m_spec);
        const auto& timing_cons = dram->m_cc_timing_cons[m_level][command].size() ? dram->m_cc_timing_cons[m_level][command] : dram->m_timing_cons[m_level][command];
        ...
        for (auto child : m_child_nodes) {
          child->update_timing_chargecache(command, addr_vec, clk);
        }
      };

      void update_powers_chargecache(int command, const AddrVec_t& addr_vec, Clk_t clk) {
        auto* dram = static_cast<DDR5ChargeCache*>(m_spec);

        if (!dram->m_drampower_enable)
          return;

        if (dram->m_cc_powers[m_level][command]) {
          dram->m_cc_powers[m_level][command](this, command, addr_vec, clk); 
        }
        ...
        if (child_id == -1) {
          for (auto child : m_child_nodes) {
            child->update_powers_chargecache(command, addr_vec, clk);
          }
        } else {
          m_child_nodes[child_id]->update_powers_chargecache(command, addr_vec, clk);
        }
      };
    };
    ...
    // ChargeCache related
    FuncMatrix<PowerFunc_t<Node>>   m_cc_powers;
    // ChargeCache related
  ...
  public:
    ...
    void issue_command(ReqBuffer::iterator& req_it) override {
      ...
      // Handle timing depending on the type of request
      if (req_it->scratchpad[0] == 1)
      {
        // This is a chargecache request
        m_channels[channel_id]->update_timing_chargecache(command, addr_vec, m_clk);
        m_channels[channel_id]->update_powers_chargecache(command, addr_vec, m_clk);
      } else {
        // Normal request
    ...
    std::vector<AddrVec_t> get_open_addresses(AddrVec_t addr_vec) override {
      int channel_id = addr_vec[m_levels["channel"]];
      return m_channels[channel_id]->get_open_addresses(addr_vec);
    };
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      // Set ChargeCache timings
      m_cc_tRCD = ceil(m_timing_vals("nRCD") * m_cc_trcd_frac);
      m_cc_tRAS = ceil(m_timing_vals("nRAS") * m_cc_tras_frac);
      m_cc_tRC = m_cc_tRAS + m_timing_vals("nRP");
      ...
      m_cc_timing_cons.resize(m_levels.size(), std::vector<std::vector<TimingConsEntry>>(m_commands.size()));
      ...
      std::vector<TimingConsInitializer> initializer = {
        ...
        {.level = "rank", .preceding = {"ACT"}, .following = {"ACT"}, .latency = V("nRRDS")},          
        {.level = "rank", .preceding = {"ACT"}, .following = {"ACT"}, .latency = V("nFAW"), .window = 4},          
        {.level = "rank", .preceding = {"ACT"}, .following = {"PREA"}, .latency = m_cc_tRAS},          

        // RAS to REF
        {.level = "rank", .preceding = {"ACT"}, .following = {"REFab", "RFMab", "DRFMab"}, .latency = m_cc_tRC},          

        // Bank
        {.level = "bank", .preceding = {"ACT"}, .following = {"ACT", "REFsb", "RFMsb", "DRFMsb"}, .latency = m_cc_tRC},
        {.level = "bank", .preceding = {"ACT"}, .following = {"RD", "RDA", "WR", "WRA"}, .latency = m_cc_tRCD},  
        {.level = "bank", .preceding = {"ACT"}, .following = {"PRE", "PREsb"}, .latency = m_cc_tRAS},
      };
      ...
      for (const auto& ts : initializer) {
        ...
        for (auto p_cmd_str : ts.preceding) {
          ...
          for (auto f_cmd_str : ts.following) {
            ...
            m_cc_timing_cons[level][p_cmd].push_back({f_cmd, ts.latency, ts.window, ts.is_sibling});
          }
        }
      }
      #undef V
    ...
    void set_actions() {
      ...
      // Rank Actions
      m_actions[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Action::Rank::PREab<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Action::Rank::REFab<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Action::Rank::REFab<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Action::Rank::REFab<DDR5ChargeCache>;
      m_actions[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Action::Rank::REFab_end<DDR5ChargeCache>;
      
      // Same-Bank Actions.
      m_actions[m_levels["bankgroup"]][m_commands["PREsb"]] = Lambdas::Action::BankGroup::PREsb<DDR5ChargeCache>;

      // We call update_timing for the banks in other BGs here
      m_actions[m_levels["bankgroup"]][m_commands["REFsb"]]  = Lambdas::Action::BankGroup::REFsb<DDR5ChargeCache>;
      m_actions[m_levels["bankgroup"]][m_commands["REFsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<DDR5ChargeCache>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb"]]  = Lambdas::Action::BankGroup::REFsb<DDR5ChargeCache>;
      m_actions[m_levels["bankgroup"]][m_commands["RFMsb_end"]]  = Lambdas::Action::BankGroup::REFsb_end<DDR5ChargeCache>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb"]] = Lambdas::Action::BankGroup::REFsb<DDR5ChargeCache>;
      m_actions[m_levels["bankgroup"]][m_commands["DRFMsb_end"]] = Lambdas::Action::BankGroup::REFsb_end<DDR5ChargeCache>;

      // Bank actions
      m_actions[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Action::Bank::ACT<DDR5ChargeCache>;
      m_actions[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Action::Bank::PRE<DDR5ChargeCache>;
      m_actions[m_levels["bank"]][m_commands["RDA"]] = Lambdas::Action::Bank::PRE<DDR5ChargeCache>;
      m_actions[m_levels["bank"]][m_commands["WRA"]] = Lambdas::Action::Bank::PRE<DDR5ChargeCache>;
    };
    ...
    void set_preqs() {
      ...
      // Rank Preqs
      m_preqs[m_levels["rank"]][m_commands["REFab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCache>;
      m_preqs[m_levels["rank"]][m_commands["RFMab"]]  = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCache>;
      m_preqs[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5ChargeCache>;

      // Same-Bank Preqs.
      m_preqs[m_levels["rank"]][m_commands["REFsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCache>;
      m_preqs[m_levels["rank"]][m_commands["RFMsb"]]  = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCache>;
      m_preqs[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5ChargeCache>;

      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["RD"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5ChargeCache>;
      m_preqs[m_levels["bank"]][m_commands["WR"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5ChargeCache>;
      m_preqs[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Preq::Bank::RequireRowOpen<DDR5ChargeCache>;
      m_preqs[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Preq::Bank::RequireBankClosed<DDR5ChargeCache>;
    };

    void set_rowhits() {
      m_rowhits.resize(m_levels.size(), std::vector<RowhitFunc_t<Node>>(m_commands.size()));

      m_rowhits[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowHit::Bank::RDWR<DDR5ChargeCache>;
      m_rowhits[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowHit::Bank::RDWR<DDR5ChargeCache>;
    }
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowOpen::Bank::RDWR<DDR5ChargeCache>;
      m_rowopens[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowOpen::Bank::RDWR<DDR5ChargeCache>;
    }
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Power::Bank::ACT<DDR5ChargeCache>;
      m_powers[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Power::Bank::PRE<DDR5ChargeCache>;
      m_powers[m_levels["bank"]][m_commands["RD"]]  = Lambdas::Power::Bank::RD<DDR5ChargeCache>;
      m_powers[m_levels["bank"]][m_commands["WR"]]  = Lambdas::Power::Bank::WR<DDR5ChargeCache>;

      // m_powers[m_levels["rank"]][m_commands["REFsb"]] = Lambdas::Power::Rank::REFsb<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["REFsb_end"]] = Lambdas::Power::Rank::REFsb_end<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["RFMsb"]] = Lambdas::Power::Rank::RFMsb<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["RFMsb_end"]] = Lambdas::Power::Rank::RFMsb_end<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Power::Rank::REFsb<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["DRFMsb_end"]] = Lambdas::Power::Rank::REFsb_end<DDR5ChargeCache>;

      m_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Power::Rank::PREA<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Power::Rank::REFab<DDR5ChargeCache>;
      m_powers[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Power::Rank::REFab<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["RFMab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Power::Rank::REFab<DDR5ChargeCache>;
      // m_powers[m_levels["rank"]][m_commands["DRFMab_end"]] = Lambdas::Power::Rank::REFab_end<DDR5ChargeCache>;

      m_powers[m_levels["rank"]][m_commands["PREsb"]] = Lambdas::Power::Rank::PREsb<DDR5ChargeCache>;

      m_cc_powers.resize(m_levels.size(), std::vector<PowerFunc_t<Node>>(m_commands.size()));

      m_cc_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<DDR5ChargeCache>;
      m_cc_powers[m_levels["bank"]][m_commands["ACT"]] = [](Node* node, int cmd, const AddrVec_t& addr, Clk_t clk) {
        Lambdas::Power::Bank::debug<DDR5ChargeCache>(node, "Incrementing ACTcc counter.", clk);
        int rank_id = Lambdas::Power::Bank::get_flat_rank_id<DDR5ChargeCache>(node);
        node->m_spec->m_power_stats[rank_id].cmd_counters[node->m_spec->m_cmds_counted("ACTcc")]++;
      };
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      Lambdas::Power::Rank::finalize_rank<DDR5ChargeCache>(rank_node, 0, AddrVec_t(), m_clk);
      ...
      double act_cc_cmd_energy = (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) 
                                      * rank_stats.cmd_counters[m_cmds_counted("ACTcc")] * m_cc_tRAS * tCK_ns / 1E3;
