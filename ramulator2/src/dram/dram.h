...
class IDRAM : public Clocked<IDRAM> {
  ...
  public:
    ...
    // Member set by RowClone-capable DRAM implementations
    Clk_t m_rc_latency = -1;            // Number of cycles needed between issuing RC command and restoration
  ...
  public:
    ...
    double s_total_request_energy = 0;    // Total energy consumed by requests (e.g., RowClone, PuM) by the device
  ...
  public:
    ...
    /**
     * @brief   Issues a command with its req iterator to the device.
     * @details
     * Issues the given command with its req iterator to the device. This function should then update
     * the states of involved nodes in the device hierarchy and their timing information.
     * 
     */
    virtual void issue_command(ReqBuffer::iterator& req_it)
    {
      // Throw a runtime error to force derived classes to implement this function
      throw std::runtime_error("issue_command() is not implemented in the derived DRAM class.");
      return;
    };
    ...
    /**
     * @brief    Populates the prequisite command for a given request
     * @details  
     * Given a request, this function should populate the prequisite 
     * command based on the current state of the device.
     * 
     */
    virtual void set_preq_command(ReqBuffer::iterator req_it)
    {
      // Throw a runtime error to force derived classes to implement this function
      throw std::runtime_error("set_preq_command() is not implemented in the derived DRAM class.");
      return;
    }
    ...
    /**
     * @brief     Checks whether the channel is ready to accept the given command.
     * @details
     * Given a channel id, this function should return whether the channel is ready to accept
     * the given command at the current clock cycle.
     * 
     */
    virtual bool check_channel_ready(int channel_id) {
    ...
    virtual std::vector<AddrVec_t> get_open_addresses(AddrVec_t addr_vec) {
      // Throw a runtime error to force derived classes to implement this function
      throw std::runtime_error("get_open_addresses() is not implemented in the derived DRAM class.");
      return {};
    };

    virtual std::vector<int> get_open_rows(AddrVec_t addr_vec) {
      throw std::runtime_error("get_open_rows() is not implemented in the derived DRAM class.");
      return {};
    };
