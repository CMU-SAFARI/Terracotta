...
struct DRAMNodeBase {
    ...
    DRAMNodeBase(T* spec, NodeType* parent, int level):
    m_spec(spec), m_parent_node(parent), m_level(level) {
      // Constructor
    };
    ...
    virtual void set_preq_command(ReqBuffer::iterator req_it, Clk_t m_clk) {
      // Throw a runtime error to force derived classes to implement this function
      throw std::runtime_error("set_preq_command() is not implemented in the derived Node class.");
    };
    ...
    bool check_node_ready(Clk_t m_clk) {
      // Check if the node is ready to accept any command at this level
      for (int cmd = 0; cmd < m_spec->m_commands.size(); cmd++) {
        if (m_cmd_ready_clk[cmd] != -1 && m_clk < m_cmd_ready_clk[cmd]) {
          // stop recursion: the check failed at this level
          return false; 
        }
      }
    ...
    std::vector<AddrVec_t> get_open_addresses(const AddrVec_t& addr_vec) {
      // If you are not in a bank node, go to the child node marked in the Addr_vec
      if (m_level != T::m_levels["bank"]) {
        int child_id = addr_vec[m_level+1];
        return m_child_nodes[child_id]->get_open_addresses(addr_vec);
      } else {
        // return all open rows in this bank
        std::vector<AddrVec_t> open_addresses;
        for (const auto& [row_id, state] : m_row_state) {
          if (state == T::m_states("Opened")) {
            AddrVec_t open_addr = addr_vec;
            open_addr[m_level] = m_node_id;
            open_addr[T::m_levels["row"]] = row_id;
            open_addresses.push_back(open_addr);
          }
        }
