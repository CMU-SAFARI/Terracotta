...
class MoPACDRAMController final : public IBHDRAMController, public Implementation {
    RAMULATOR_REGISTER_IMPLEMENTATION(IBHDRAMController, MoPACDRAMController, "MoPACDRAMController", "MoPAC DRAM controller.")
...
public:
    void init() override {
        ...
        if (!m_prac) {
            throw ConfigurationError("[MoPACDRAMController] The MoPAC PRAC plugin is required but not configured!");
        }
    };
    ...
    void tick() override {
        ...
        if (request_found) {
            // Modify the issue command to send the entire req_it
