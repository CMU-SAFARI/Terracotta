...
DECLARE_DEBUG_FLAG(PuMDRAMController);
// ENABLE_DEBUG_FLAG(PuMDRAMController);

class PuMDRAMController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, PuMDRAMController, "PuMController", "A PuM Capable DRAM controller.");
  private:
    ...
    ReqBuffer m_pum_buffer;              // PuM request buffer
    ...
    size_t s_num_pum_reqs = 0;

    size_t s_read_latency = 0;
    size_t s_pum_latency = 0;
    float s_avg_read_latency = 0;
    float s_avg_pum_latency = 0;

  public:
    void init() override {
      m_wr_low_watermark =  param<float>("wr_low_watermark").desc("Threshold for switching back to read mode.").default_val(0.2f);
      m_wr_high_watermark = param<float>("wr_high_watermark").desc("Threshold for switching to write mode.").default_val(0.8f);
      controller_latency = param<int>("controller_latency").desc("Controller internal latency in cycles.").default_val(0);
      ...
      m_logger = Logging::create_logger("PuMDRAMController");
    };
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      register_stat(s_num_pum_reqs).name("num_pum_reqs_{}", m_channel_id);

      register_stat(s_read_latency).name("read_latency_{}", m_channel_id);
      register_stat(s_pum_latency).name("pum_latency_{}", m_channel_id);
      register_stat(s_avg_read_latency).name("avg_read_latency_{}", m_channel_id);
      register_stat(s_avg_pum_latency).name("avg_pum_latency_{}", m_channel_id);
    };
    ...
    bool send(Request& req) override {
      ...
      // Consider RowClone requests to be read requests
      // Else, enqueue them to corresponding buffer based on request type id
      bool is_success = false;
      req.arrive = m_clk;
      int type_id = req.type_id;
      if (type_id == Request::Type::Read || type_id == Request::Type::Write) {
        throw std::runtime_error("We do not support normal read requests in PuM controller!");
      } else if (type_id == Request::Type::Write) {
        is_success = m_write_buffer.enqueue(req);
      } else if (type_id == Request::Type::RowClone || 
                 type_id == Request::Type::NOT || 
                 type_id == Request::Type::AND_OR || 
                 type_id == Request::Type::XOR_XNOR || 
                 type_id == Request::Type::NAND_NOR) {
        is_success = m_pum_buffer.enqueue(req);
      } else {
    ...
    void tick() override {
      ...
      if (request_found) {
        ...
        // Book-keeping of RowClone requests
        ...
        if (req_it->is_done) {
          if (req_it->type_id == Request::Type::Read) {
          ...
          } else if (req_it->type_id == Request::Type::RowClone || 
            ...
            req_it->depart = m_clk + controller_latency;
            pending.push_back(*req_it);
          }
          buffer->remove(req_it);
        } else {
          // If command is opening and the request is not a part of the active buffer, move it to the active buffer
          if (m_dram->m_command_meta(req_it->command).is_opening && buffer != &m_active_buffer) {
    ...
    */
          ...
          if (req.depart - req.arrive > 1) {
            ...
            if (req.type_id == Request::Type::Read) {
              s_read_latency += req.depart - req.arrive;
                       ...
                       req.type_id == Request::Type::NOT || 
                       req.type_id == Request::Type::AND_OR || 
                       req.type_id == Request::Type::XOR_XNOR || 
                       req.type_id == Request::Type::NAND_NOR) {
              // PuM requests
              s_pum_latency += req.depart - req.arrive;
            }
          }
    ...
    bool schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer) {
      ...
      if (!request_found) {
        ...
        if (m_priority_buffer.size() != 0 && m_active_buffer.size() == 0) {
          // Is an act buffer request available?
        ...
        // If there is no potential request in the priority buffer, check the PuM buffer
        if (!request_found && m_priority_buffer.size() == 0 && m_pum_buffer.size() != 0) {
          req_buffer = &m_pum_buffer;
          req_it = m_pum_buffer.begin();
    ...
    void finalize() override {
      ...
      s_avg_pum_latency = (float) s_pum_latency / (float) s_num_pum_reqs;
