...
class TerracottaMoPAC : public IControllerPlugin, public Implementation, public IPRAC {
    RAMULATOR_REGISTER_IMPLEMENTATION(IControllerPlugin, TerracottaMoPAC, "TerracottaMoPAC", "TerracottaMoPAC.")
...
private:
    DeviceConfig m_cfg;
    std::vector<TerracottaMoPAC::PerBankCounters> m_bank_counters;
    std::vector<int> m_same_bank_offsets;

    std::vector<bool> m_needs_counter_update;
...
public:
    void init() override { 
        ...
        m_pr_threshold = param<float>("update_probability").desc("Probability of updating the PRAC counters.").required();
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
        ...
        m_needs_counter_update.resize(m_cfg.m_num_banks, false);
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
        ...
        // If ACT, RDA, WRA, RD, WR, PRE check the m_needs_counter_update flag for this bank to set scratchpad[0]
        if (req.command == m_cfg.m_dram->m_commands("ACT") ||
            req.command == m_cfg.m_dram->m_commands("RDA") || 
            req.command == m_cfg.m_dram->m_commands("WRA") || 
            req.command == m_cfg.m_dram->m_commands("RD") || 
            req.command == m_cfg.m_dram->m_commands("WR") ||
            req.command == m_cfg.m_dram->m_commands("PRE")) {
            if (m_needs_counter_update[m_cfg.get_flat_bank_id(req)]) {
                req.scratchpad[0] = 0;
            } else {
                req.scratchpad[0] = 1;
            }
        ...
        // Update counters if you plan to update them
        ...
        // If request is an PRE, probabilistically assign req.scratchpad[0] to 0 (update counters) or 1 (do not update counters)
        // If you do not want to update counters, set the m_needs_counter_update flag to false for this bank
        if (req.command == m_cfg.m_dram->m_commands("PRE")) {
            if (m_distribution(m_generator) < m_pr_threshold) {
                m_needs_counter_update[m_cfg.get_flat_bank_id(req)] = true;
            } else {
                m_needs_counter_update[m_cfg.get_flat_bank_id(req)] = false;
            }
        }
    }
    ...
    void update_state_machine(bool request_found, const Request& req) {
        ...
        case ABOState::NORMAL:
            if (m_is_abo_needed) {
                if (m_debug) {
                    std::printf("[TerracottaMoPAC] [%lu] <%s> Asserting ALERT_N.\n", m_clk, state_names[cur_state].c_str());
                }
        ...
        case ABOState::PRE_RECOVERY:
            if (request_found && req.command == cmd_prea) {
                if (m_debug) {
                    std::printf("[TerracottaMoPAC] [%lu] <%s> Received PREA.\n", m_clk, state_names[cur_state].c_str());
                }
            }
...
private:
        ...
        void init_dram_params(IDRAM* dram) {
            ...
            for (auto& h : handlers) {
                if (!dram->m_commands.contains(h.cmd_name)) {
                    throw ConfigurationError("[TerracottaMoPAC] Command {} does not exist.", h.cmd_name);
                }
    ...
    private:
        ...
        void process_act(const Request& req) {
            if (req.scratchpad[0] == 1) {
                if (m_debug) {
                    std::printf("[TerracottaMoPAC] [%d] [ACT] Skipping counter update for row %d.\n",
                        m_bank_id, req.addr_vec[m_cfg.m_row_level]);
                }
                return;
            }
            ...
            m_counters[row_addr]++;
            if (m_debug) {
                std::printf("[TerracottaMoPAC] [%d] [ACT] Row: %d Act: %u\n",
        ...
        void process_rfm(const Request& req) {
            ...
            if (act_max == m_counters.end()) {
                if (m_debug) {
                    std::printf("[TerracottaMoPAC] [%d] [RFM] No critical row.\n", m_bank_id);
                }
                return;
            }
            if (m_debug) {
                std::printf("[TerracottaMoPAC] [%d] [RFM] Row: %d Act: %u\n",
