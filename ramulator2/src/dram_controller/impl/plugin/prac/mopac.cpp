...
class MoPAC : public IControllerPlugin, public Implementation, public IPRAC {
    RAMULATOR_REGISTER_IMPLEMENTATION(IControllerPlugin, MoPAC, "MoPAC", "MoPAC.")
...
public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
        ...
        // If request is an ACT, probabilistically assign req.scratchpad[0] to 0 (update counters) or 1 (do not update counters)
        ...
        // If RDA, WRA, RD, WR, PRE check the m_needs_counter_update flag for this bank to set scratchpad[0]
        if (req.command == m_cfg.m_dram->m_commands("RDA") || 
        ...
        // If ACT, check m_needs_counter_update to handle requests with bank or bankgroup wildcards
    ...
    void update_state_machine(bool request_found, const Request& req) {
        ...
        case ABOState::NORMAL:
            if (m_is_abo_needed) {
                if (m_debug) {
                    std::printf("[MoPAC] [%lu] <%s> Asserting ALERT_N.\n", m_clk, state_names[cur_state].c_str());
                }
        ...
        case ABOState::PRE_RECOVERY:
            if (request_found && req.command == cmd_prea) {
                if (m_debug) {
                    std::printf("[MoPAC] [%lu] <%s> Received PREA.\n", m_clk, state_names[cur_state].c_str());
                }
            }
        ...
        void init_dram_params(IDRAM* dram) {
            ...
            for (auto& h : handlers) {
                if (!dram->m_commands.contains(h.cmd_name)) {
                    throw ConfigurationError("[MoPAC] Command {} does not exist.", h.cmd_name);
                }
    ...
    private:
        ...
        void process_act(const Request& req) {
            if (req.scratchpad[0] == 1) {
                if (m_debug) {
                    std::printf("[MoPAC] [%d] [ACT] Skipping counter update for row %d.\n",
            ...
            m_counters[row_addr]++;
            if (m_debug) {
                std::printf("[MoPAC] [%d] [ACT] Row: %d Act: %u\n",
        ...
        void process_rfm(const Request& req) {
            ...
            if (act_max == m_counters.end()) {
                if (m_debug) {
                    std::printf("[MoPAC] [%d] [RFM] No critical row.\n", m_bank_id);
                }
                return;
            }
            if (m_debug) {
                std::printf("[MoPAC] [%d] [RFM] Row: %d Act: %u\n",
