...
class CommandCounter : public IControllerPlugin, public Implementation {
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      int nCommands = m_dram->m_commands.size();
      for (int i = 0; i < nCommands; i++) {
        m_command_counters[i] = 0;
      }
    };
    ...
    void finalize() override {
      ...
      // Adds DRAM standard impl and number of DRAM command to the output file 
      output << fmt::format("nCommands, {}", m_dram->m_commands.size()) << std::endl;
