...
class ChargeCacheDRAMController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, ChargeCacheDRAMController, "ChargeCacheController", "A ChargeCache DRAM controller.");
  ...
  class HCRAC {
    ...
    uint64_t num_rank_bits, num_bg_bits, num_bank_bits, num_row_bits;
    uint64_t rank_idx, bg_idx, bank_idx, row_idx;
  ...
  private:
    ...
    std::vector<int> m_bank_act_core_map; // Map to track which core activated which bank
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      int num_flat_banks = m_dram->get_level_size("rank") * m_dram->get_level_size("bankgroup") * m_dram->get_level_size("bank");
      m_bank_act_core_map.resize(num_flat_banks, -1);
      // ChargeCache setup
    ...
    void tick() override {
      ...
      if (request_found) {
        ...
        // 1.0 Handle ACTs — HCRAC lookup ONLY for ACT commands
        if (command == m_dram->m_commands("ACT") && 
          ...
          // 1.1 Update Active Bank Map
          m_bank_act_core_map[flat_bank_id] = req_it->source_id;

          // 1.2 Perform HCRAC lookup
          int source_id = req_it->source_id;
          HCRAC* hcrac = m_hcrac[source_id];
        ...
        if (command == m_dram->m_commands("PRE") || command == m_dram->m_commands("RDA") || command == m_dram->m_commands("WRA")) {
          int core_id = m_bank_act_core_map[flat_bank_id];

          if (core_id != -1) {
            HCRAC* hcrac = m_hcrac[core_id];
            std::vector<AddrVec_t> open_addresses = get_open_addresses(req_it->addr_vec);

            for (auto& addr_vec : open_addresses) {
              if (m_is_debug) {
                std::cout << "Updating HCRAC for core " << core_id << " Address Vector: [";
              ...
              if (std::get<1>(result))
                s_hcrac_evictions_per_core[core_id]++;
              if (std::get<0>(result))
                s_hcrac_updates_per_core[core_id]++;
            }

            // Clear the bank to core mapping
            m_bank_act_core_map[flat_bank_id] = -1;
          }
        }

        // 3.0 Handle PREA commands
        if (command == m_dram->m_commands("PREA")) {
          ...
          for (int r = 0; r < num_ranks; r++) {
            for (int bgi = 0; bgi < num_bgs; bgi++) {
              for (int b = 0; b < num_banks; b++) {
                int fbi = r * (num_bgs * num_banks) + bgi * num_banks + b;
                int core_id = m_bank_act_core_map[fbi];
                ...
                std::vector<AddrVec_t> open_addresses = get_open_addresses(pre_addr_vec);
    ...
    */
    ...
    void finalize() override {
      ...
      for (int i = 0; i < m_num_cores; i++) {
        ...
        s_hcrac_hits += s_hcrac_hits_per_core[i];
        s_hcrac_misses += s_hcrac_misses_per_core[i];
        s_hcrac_updates += s_hcrac_updates_per_core[i];
        s_hcrac_invalidations += s_hcrac_invalidations_per_core[i];
        s_hcrac_evictions += s_hcrac_evictions_per_core[i];
      }
