...
class SubarrayClosedRowPolicy : public IRowPolicy, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IRowPolicy, SubarrayClosedRowPolicy, "SubarrayClosedRowPolicy", "Subarray Closed Row Policy.")
  private:
    IDRAM* m_dram;
    ...
    int m_bankgroup_level = -1;
    int m_bank_level = -1;
    int m_subarray_level = -1;
    int m_row_level = -1;
    int m_num_ranks = -1;
    int m_num_bankgroups = -1;
    int m_num_banks = -1;
    int m_num_subarrays = -1;
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_bankgroup_level = m_dram->m_levels("bankgroup");
      m_bank_level = m_dram->m_levels("bank");
      m_subarray_level = m_dram->m_levels("subarray");
      m_row_level = m_dram->m_levels("row");
      ...
      m_col_accesses.resize(m_num_subarrays * m_num_banks * m_num_bankgroups * m_num_ranks, 0);
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
        ...
        if (req_it->addr_vec[m_bankgroup_level] == -1 && req_it->addr_vec[m_bank_level] == -1) {  // all bank closes
          for (int b = 0; b < m_num_banks; b++) {
            for (int bg = 0; bg < m_num_bankgroups; bg++) {
              ...
              int rank_id = req_it->addr_vec[m_rank_level];
              int flat_sa_id = sa + b * m_num_subarrays + bg * m_num_banks * m_num_subarrays + rank_id * m_num_banks * m_num_bankgroups * m_num_subarrays;
              m_col_accesses[flat_sa_id] = 0;
              }
            }
          }
        } else if (req_it->addr_vec[m_bankgroup_level] == -1) {  // same bank closes
          for (int bg = 0; bg < m_num_bankgroups; bg++) {
            for (int sa = 0; sa < m_num_subarrays; sa++) {
              int bank_id = req_it->addr_vec[m_bank_level];
              int rank_id = req_it->addr_vec[m_rank_level];
              int flat_sa_id = sa + bank_id * m_num_subarrays + bg * m_num_banks * m_num_subarrays + rank_id * m_num_banks * m_num_bankgroups * m_num_subarrays;

              m_col_accesses[flat_sa_id] = 0;
            }
          }
        } else if (req_it->addr_vec[m_subarray_level] == -1) {  // single bank closes  (PRE, VRR)
          for (int sa = 0; sa < m_num_subarrays; sa++) {
            int bank_id = req_it->addr_vec[m_bank_level];
            int bg_id = req_it->addr_vec[m_bankgroup_level];
            int rank_id = req_it->addr_vec[m_rank_level];
            int flat_sa_id = sa + bank_id * m_num_subarrays + bg_id * m_num_banks * m_num_subarrays + rank_id * m_num_banks * m_num_bankgroups * m_num_subarrays;

            m_col_accesses[flat_sa_id] = 0;
          }
        } else {  // Single subarray closes (PRE_SA)
          int flat_sa_id = req_it->addr_vec[m_subarray_level] + 
                           req_it->addr_vec[m_bank_level] * m_num_subarrays + 
                           req_it->addr_vec[m_bankgroup_level] * m_num_banks * m_num_subarrays + 
                           req_it->addr_vec[m_rank_level] * m_num_banks * m_num_bankgroups * m_num_subarrays;
      ...
      } else if (m_dram->m_command_meta(req_it->command).is_accessing)  // RD or WR
      {
        ...
        m_col_accesses[flat_sa_id]++;

        if (m_col_accesses[flat_sa_id] >= m_cap) {
          Request req(req_it->addr_vec, m_PRE_req_id);
          m_ctrl->priority_send(req);
          m_col_accesses[flat_sa_id] = 0;
          s_num_close_reqs++;
