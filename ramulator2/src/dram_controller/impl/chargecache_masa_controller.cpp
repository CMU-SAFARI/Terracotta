...
class ChargeCacheMASADRAMController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, ChargeCacheMASADRAMController, "ChargeCacheMASAController", "A ChargeCache+MASA DRAM controller.");
  ...
  class HCRAC {
    ...
    uint64_t num_rank_bits, num_bg_bits, num_bank_bits, num_sa_bits, num_row_bits;
    uint64_t rank_idx, bg_idx, bank_idx, sa_idx, row_idx;
    ...
    HCRAC(int num_entries, int num_ways, int caching_duration, IDRAM* dram) : entries(num_entries / num_ways, std::vector<entry>(num_ways)),
      ...
      num_rank_bits = dram->get_level_size("rank") > 1 ? (uint64_t)std::ceil(std::log2(dram->get_level_size("rank"))) : 0;
      num_bg_bits = dram->get_level_size("bankgroup") > 1 ? (uint64_t)std::ceil(std::log2(dram->get_level_size("bankgroup"))) : 0;
      num_bank_bits = dram->get_level_size("bank") > 1 ? (uint64_t)std::ceil(std::log2(dram->get_level_size("bank"))) : 0;
      num_sa_bits = dram->get_level_size("subarray") > 1 ? (uint64_t)std::ceil(std::log2(dram->get_level_size("subarray"))) : 0;
      num_row_bits = dram->get_level_size("row") > 1 ? (uint64_t)std::ceil(std::log2(dram->get_level_size("row"))) : 0;

      rank_idx = dram->m_levels("rank");
      bg_idx = dram->m_levels("bankgroup");
      bank_idx = dram->m_levels("bank");
    ...
    uint64_t get_hcrac_addr (AddrVec_t addr_vec) {
      uint64_t hcrac_addr = 0;

      hcrac_addr = addr_vec[row_idx];
      hcrac_addr = (hcrac_addr << num_sa_bits) | addr_vec[sa_idx];
      hcrac_addr = (hcrac_addr << num_bank_bits) | addr_vec[bank_idx];
      hcrac_addr = (hcrac_addr << num_bg_bits) | addr_vec[bg_idx];
      hcrac_addr = (hcrac_addr << num_rank_bits) | addr_vec[rank_idx];
  ...
  private:
    ...
    std::vector<size_t> s_hcrac_hits_per_core;
    std::vector<size_t> s_hcrac_misses_per_core;
    std::vector<size_t> s_hcrac_updates_per_core;
    std::vector<size_t> s_hcrac_invalidations_per_core;
    std::vector<size_t> s_hcrac_evictions_per_core;
  ...
  public:
    void init() override {
      ...
      m_num_entries_per_core = param_group("Config").param<int>("num_entries_per_core").required().desc("Number of entries in the ChargeCache.");
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      for (int i = 0; i < m_num_cores; i++) {
        m_hcrac.push_back(new HCRAC(m_num_entries_per_core, m_set_associativity, m_caching_duration, m_dram));
      }
      m_hcrac_inv_interval = JEDEC_rounding_DDR5(m_caching_duration * 1000000 / m_num_entries_per_core, m_dram->m_timing_vals("tCK_ps"));
      // ChargeCache setup
      ...
      s_hcrac_hits_per_core.resize(m_num_cores, 0);
      s_hcrac_misses_per_core.resize(m_num_cores, 0);
      s_hcrac_updates_per_core.resize(m_num_cores, 0);
      s_hcrac_invalidations_per_core.resize(m_num_cores, 0);
      s_hcrac_evictions_per_core.resize(m_num_cores, 0);

      for (size_t core_id = 0; core_id < m_num_cores; core_id++) {
        register_stat(s_hcrac_hits_per_core[core_id]).name("hcrac_hits_core_{}", core_id);
        register_stat(s_hcrac_misses_per_core[core_id]).name("hcrac_misses_core_{}", core_id);
        register_stat(s_hcrac_updates_per_core[core_id]).name("hcrac_updates_core_{}", core_id);
        register_stat(s_hcrac_invalidations_per_core[core_id]).name("hcrac_invalidations_core_{}", core_id);
        register_stat(s_hcrac_evictions_per_core[core_id]).name("hcrac_evictions_core_{}", core_id);
      }
    ...
    void tick() override {
      ...
      if (m_clk % m_hcrac_inv_interval == 0)
      {
        ...
        uint64_t set_idx = m_hcrac_inv_idx / m_set_associativity;
        uint64_t way_idx = m_hcrac_inv_idx % m_set_associativity;
        for (int i = 0; i < m_num_cores; i++) {
          s_hcrac_invalidations_per_core[i]++;
      ...
      bool has_valid_source = (source_id >= 0 && source_id < m_num_cores);

      if (request_found && has_valid_source) {
        hcrac = m_hcrac[req_it->source_id];

        // 2.0.1. Test if the request belongs to the HCRAC
        if ((req_it->type_id == Request::Type::Read || req_it->type_id == Request::Type::Write) 
          ...
          if (lookup) {
            if (m_is_debug) {
              std::cout << "HCRAC hit for core " << source_id << std::endl;
            }
            s_hcrac_hits_per_core[source_id]++;
            req_it->scratchpad[0] = 1; // HCRAC hit
          } else {
            if (m_is_debug) {
              std::cout << "HCRAC miss for core " << source_id << std::endl;
            }
            s_hcrac_misses_per_core[source_id]++;
            req_it->scratchpad[0] = 0; // HCRAC miss
          }
        } else {
          req_it->scratchpad[0] = 0; // Normal request
        }
      }

      // 2.0.2. If the request precharges, add all open rows to the HCRAC
      // This must run even for priority close requests (source_id == -1) from SubarrayClosedRowPolicy,
      // because the physical row charge is retained regardless of which core accessed it.
      if (request_found) {
        ...
        if (is_precharge || is_bank_precharge || is_all_bank_precharge) {
          // Build the list of (addr_vec with row filled) to cache
          std::vector<AddrVec_t> precharge_addr_list;

          if (is_precharge) {
            // PRE_SA/RDA/WRA closes a single subarray; addr_vec has the subarray filled in
            ...
            for (auto row : open_rows) {
              ...
              precharge_addr_list.push_back(hcrac_addr_vec);
            }
            ...
            // PREA closes all banks in all ranks — iterate all ranks/bgs/banks/subarrays
            int num_ranks = m_dram->get_level_size("rank");
            int num_bgs = m_dram->get_level_size("bankgroup");
            int num_banks = m_dram->get_level_size("bank");

            for (int r = 0; r < num_ranks; r++) {
              for (int bgi = 0; bgi < num_bgs; bgi++) {
                for (int b = 0; b < num_banks; b++) {
                  AddrVec_t pre_addr_vec = req_it->addr_vec;
                  pre_addr_vec[m_dram->m_levels("rank")] = r;
                  pre_addr_vec[m_dram->m_levels("bankgroup")] = bgi;
                  pre_addr_vec[m_dram->m_levels("bank")] = b;
          ...
          // Update the HCRAC(s)
          if (has_valid_source) {
            // Known source core — update only that core's HCRAC
            for (auto& addr_vec : precharge_addr_list) {
              if (m_is_debug) {
                std::cout << "Updating HCRAC for core " << source_id << " Address Vector: [";
                for (const auto& addr : addr_vec) { std::cout << addr << " "; }
                std::cout << "]" << std::endl;
              }
              std::tuple<bool, bool> result = m_hcrac[source_id]->update(addr_vec);
              if (std::get<1>(result))
                s_hcrac_evictions_per_core[source_id]++;
              if (std::get<0>(result))
                s_hcrac_updates_per_core[source_id]++;
            }
          } else {
            // No valid source (e.g., priority close from SubarrayClosedRowPolicy)
            // Update ALL cores' HCRACs — the charged row benefits any future access
            for (auto& addr_vec : precharge_addr_list) {
              if (m_is_debug) {
                std::cout << "Updating HCRAC for ALL cores, Address Vector: [";
                for (const auto& addr : addr_vec) { std::cout << addr << " "; }
                std::cout << "]" << std::endl;
              }
              for (int c = 0; c < m_num_cores; c++) {
                std::tuple<bool, bool> result = m_hcrac[c]->update(addr_vec);
                if (std::get<1>(result))
                  s_hcrac_evictions_per_core[c]++;
                if (std::get<0>(result))
                  s_hcrac_updates_per_core[c]++;
              }
            }
          }
        }
